...
Each student works on a different file for a specific part of the website. The most senior student can serve as the team leader (responsible for integration and deployment) unless the teacher prefers to take on that role for the whole class. The HTML Template Engine library will handle assembling all the pieces
...
3. Each contributor will need to clone the new forked repository and develop a part of the website that is coordinated with the group. Each project is divided into pieces inside the `website/templates/` directory. Once everyone has their editor open, run the project in the terminal with this command:
...
4. To start, each member should put their name in the file they have been assigned, `push` back to the repository, and `pull` to see the others changes. Maintaining clear communication about the files being used will make the project run smoothly 🙂
